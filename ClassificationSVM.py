...
data = pandas.read_csv('C:/Users/user/Documents/train.csv', nrows=1000, error_bad_lines=False, warn_bad_lines=False)
...
SVM = sklearn.model_selection.GridSearchCV(sklearn.svm.SVC(shrinking=True, probability=True, class_weight={1: 1/numpy.average(y_train)}), {'kernel':['rbf','poly'], 'degree':[2,3], 'C':[1,2]}, cv=3).fit(x_train, y_train).best_estimator_

SVM_predict = SVM.predict(x_test)
SVM_predict_proba = SVM.predict_proba(x_test)
SVM_ROC = sklearn.metrics.roc_curve(y_test, SVM_predict_proba[:,1])
SVM_confusion = (sklearn.metrics.confusion_matrix(y_test, SVM_predict)*len(y_test)**(-1)).round(2)
SVM_report = sklearn.metrics.classification_report(y_test, SVM_predict, target_names = ['not click', 'click'], digits=4)

figure, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figsize=(15,3))
#SVM Confusion Matrix
plt.sca(ax1)
plt.imshow(SVM_confusion, interpolation='nearest', cmap=plt.cm.YlGn)
plt.title('Confusion Matrix of SVM')
plt.colorbar()
plt.xticks(range(2))
plt.yticks(range(2))
for i, j in itertools.product(range(2),range(2)):
    plt.text(j, i, SVM_confusion[i, j], horizontalalignment="center", color="black", size='xx-large')
...
#All ROC
plt.sca(ax2)
plt.plot(SVM_ROC[0], SVM_ROC[1], 'y')
...
print('SVM parameters with cross-validation')

...
data = pandas.read_csv('C:/Users/user/Documents/train.csv', nrows=10000, error_bad_lines=False, warn_bad_lines=False)
...
Log = sklearn.linear_model.LogisticRegressionCV(cv=3).fit(x_train, y_train)

Log_predict = Log.predict(x_test)
Log_predict_proba = Log.predict_proba(x_test)
Log_ROC = sklearn.metrics.roc_curve(y_test, Log_predict_proba[:,1])
Log_confusion = (sklearn.metrics.confusion_matrix(y_test, Log_predict)*len(y_test)**(-1)).round(4)
Log_report = sklearn.metrics.classification_report(y_test, Log_predict, target_names = ['not click', 'click'], digits=4)

figure, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figsize=(15,3))

#LogisticRegressionCV Confusion Matrix
plt.sca(ax1)
plt.imshow(Log_confusion, interpolation='nearest', cmap=plt.cm.YlGn)
plt.title('Confusion Matrix of LogisticRegressionCV')
plt.colorbar()
plt.xticks(range(2))
plt.yticks(range(2))
for i, j in itertools.product(range(2),range(2)):
    plt.text(j, i, Log_confusion[i, j], horizontalalignment="center", color="black", size='xx-large')
...
#ROC
plt.sca(ax2)
plt.plot(Log_ROC[0], Log_ROC[1], color='b', label='Log')
plt.legend(loc=4)
...
print('Coef')
print(pandas.DataFrame(numpy.array([Log.coef_[0].round(4)]).T, columns=['Log'], index=keys))

print('LogisticRegressionCV report')
print(Log_report)

#--------------------------------------
# date: 2070410             
# title: logistic regression on SparkR 
# data: sample 10000        
#---------------------------------------
# 1. read file , read_csv --> createDataFrame
...
#               效果未如理想
#----------------------------
.libPaths("/usr/local/spark/R/lib/")
...
dat = spark_read_csv(sc, 'sample', "sample10000.csv")

a1 = dat %>% group_by(srch_id) %>% summarize(count = sum(n())) %>% collect()
...
###########################
# 搵AIC最低
formu = summary(step(spark.glm(click_bool~., data = dataSelect, family = 'binomial'), direction = "backward"))$call
